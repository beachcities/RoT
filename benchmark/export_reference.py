...
REFERENCE = [
    ...
    # 本測定（confirmatory）反復1。
    "run_20260827T060549Z.json",
    # 本測定（confirmatory）反復2。
    "run_20260827T144958Z.json",
]
...
NOTES = {
    ...
    "run_20260826T132218Z.json": "**計器v2.0 のパイロット**（タグ `instrument-v2.0`、"
                                 ...
                                 "200標本・エラー0・11区間",
    "run_20260827T060549Z.json": "**本測定（confirmatory）の反復1**"
                                 "（AUDITED_COMMIT `0b8c3ae7ddf822aafbcde9034a41d58ae58ed699`、"
                                 "`fingerprint.git.commit` の一致を確認済み）。"
                                 "第二凍結 `paper/notes/instrument-v2-freeze2.md` 第3節に"
                                 "基づく——**パイロットと同一の200標本設計を、別の "
                                 "master seed 帯で反復**（組 `v4_distribution_m1`、"
                                 "**S1〜S12=20261001〜20261012**）。V/H/γ・完全交差・"
                                 "山推定・棄権判定はパイロットから不変。"
                                 "**パイロットの n=200 は本測定の推定値に pool しない**"
                                 "（判定規則の制定に使ったデータのため。再現性評価の"
                                 "外部参照としてのみ併記）。200標本・エラー0・10区間。"
                                 "**区間5が VM ごと落ち、VM 側で生成された3標本が"
                                 "退避されず失われた**——第二凍結 第4節の「標本0で落ちた"
                                 "区間は破棄し未取得分を次区間で走行」に該当し、"
                                 "取得済み標本の差し替えは行っていない",
    "run_20260827T144958Z.json": "**本測定（confirmatory）の反復2**"
                                 "（AUDITED_COMMIT `0b8c3ae7ddf822aafbcde9034a41d58ae58ed699`、"
                                 "`fingerprint.git.commit` の一致を確認済み）。"
                                 "組 `v4_distribution_m2`、**S1〜S12=20261101〜20261112**。"
                                 "反復1（`run_20260827T060549Z`）と合わせて "
                                 "**本測定 n=400**。設計・V/H/γ・完全交差・山推定・"
                                 "棄権判定は反復1から不変で、違うのは seed 帯だけ。"
                                 "200標本・エラー0・11区間（区間1が標本0で落ちて破棄）",
}
